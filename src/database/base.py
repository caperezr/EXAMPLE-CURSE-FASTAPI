...
class BaseQuery(Query):
    ...
    def get_or_404(self, ident, description=None):
        """Like :meth:`get` but CustomExceptions with 404 if not found instead of
        returning ``None``.
        """
        rv = self.get(ident)
        if rv is None:
          raise CustomException(status_code=404, detail='No se encuentra disponible ó ha sido eliminado.')
        return rv

    def first_or_404(self, description=None):
        """Like :meth:`first` but CustomExceptions with 404 if not found instead
        of returning ``None``.
        """
        rv = self.first()
        if rv is None:
    ...
    def paginate(
        ...
        * ``page`` or ``per_page`` are not ints.
        ...
        if page < 1:
          if error_out:
            CustomException(status_code=400)
          else:
            page = 1
